...
class GildedRoseTest {
    ...
    void shouldDecreaseSellInWhenAnyNameAndQualityIsZero() {
        Item item = new ClassicItem("Foo", 1, 0);
        GildedRose app = buildGildedRose(item);
        app.updateQuality();
        assertThat(item).usingRecursiveComparison().isEqualTo(new ClassicItem("Foo", 0, 0));
    }

    @Test
    void shouldDecreaseSellInAndQualityWhenAnyNameAndQualityIsSuperiorToZero() {
        Item item = new ClassicItem("Foo", 1, 1);
        GildedRose app = buildGildedRose(item);
        app.updateQuality();
        assertThat(item).usingRecursiveComparison().isEqualTo(new ClassicItem("Foo", 0, 0));
    }
    ...
    private Item createSulfura(int quality, int sellIn) {
        return new Sulfura(sellIn, quality);
    }
    ...
    void shouldDecreaseSellInWhenAnyNameAndSellInLowerOrEqualTo0AndQualityIs0() {
        Item item = new ClassicItem("Foo", 0, 0);
        GildedRose app = buildGildedRose(item);
        app.updateQuality();
        assertThat(item).usingRecursiveComparison().isEqualTo(new ClassicItem("Foo", -1, 0));
    }

    @Test
    void shouldDecreaseSellInAndDecreaseQualityBy2WhenAnyNameAndSellInLowerOrEqualTo0() {
        Item item = new ClassicItem("Foo", 0, 5);
        GildedRose app = buildGildedRose(item);
        app.updateQuality();
        assertThat(item).usingRecursiveComparison().isEqualTo(new ClassicItem("Foo", -1, 3));
    }
